...
<style>
  ...
  .header h1 {{ color: #60A5FA; margin: 0; font-size: 22px; letter-spacing: 1px; }}
  .header p {{ color: rgba(255,255,255,0.5); margin: 6px 0 0; font-size: 13px; }}
  .content {{ padding: 32px; }}
  .consigliato-box {{ background: linear-gradient(135deg, #0a1628, #0d2145); border: 2px solid #60A5FA;
    border-radius: 8px; padding: 20px 24px; margin-bottom: 32px; }}
  .consigliato-box .badge {{ background: #60A5FA; color: #000; font-weight: 700; font-size: 12px;
    padding: 3px 12px; border-radius: 20px; display: inline-block; margin-bottom: 10px; }}
  .consigliato-box h2 {{ color: #60A5FA; margin: 0 0 8px; font-size: 16px; }}
  ...
  .article-header {{ background: #0a1628; padding: 14px 20px; }}
  .article-num {{ background: #60A5FA; color: #000; font-weight: 700; font-size: 13px; padding: 3px 10px; border-radius: 3px; }}
  .article-body {{ padding: 24px; }}
  .article-body h1 {{ font-size: 20px; color: #111; margin: 0 0 16px; line-height: 1.3; }}
  .article-body h2 {{ font-size: 16px; color: #222; margin: 20px 0 10px; border-left: 3px solid #60A5FA; padding-left: 10px; }}
  .article-body p {{ font-size: 14px; color: #444; line-height: 1.7; margin: 0 0 12px; }}
  .seo-block {{ background: #f0f4ff; border: 1px solid #c0d0e8; border-radius: 4px; padding: 12px 16px; margin-top: 16px; font-size: 12px; color: #444; }}
  .image-block {{ background: #f0f4ff; border: 1px solid #c0d0e8; border-radius: 4px; padding: 12px 16px; margin-top: 12px; font-size: 12px; }}
  .image-block strong {{ color: #1a4a8a; display: block; margin-bottom: 6px; }}
  .prompt {{ font-family: monospace; background: #e8eeff; padding: 6px 10px; border-radius: 3px; display: block; margin-top: 4px; color: #223; }}
  .firefly-link {{ display: inline-block; margin-top: 8px; background: #1a4a8a; color: #fff; padding: 4px 12px; border-radius: 3px; text-decoration: none; font-size: 11px; font-weight: 700; }}
...
<div class="container">
  <div class="header">
    <h1>🖥️ IncastroPC Mini PC News</h1>
  ...
  <div class="footer">Generato da IncastroPC Mini PC Bot · {date}</div>
</div>
</body>
</html>
"""
...
def send_digest(raw_text):
    ...
    msg["Subject"] = "🖥️ IncastroPC Mini PC News — " + today
    msg["From"] = EMAIL_FROM
    msg["To"] = EMAIL_TO
